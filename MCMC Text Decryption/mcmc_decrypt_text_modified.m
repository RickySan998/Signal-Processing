function [decrypted_txt, decrypt_key] = mcmc_decrypt_text_modified(encrypted_txt, pr_trans)
    ...
    % add MAP implementation; create 2 arrays, 1 to store the keys obtained
    % after 1 run of metropolis algorithm, the other one to store the
    % occurences of these keys
    keylist = [];
    occurencelist = [];
    ...
    % run mcmc for 3000 iterations
        ...
        if isempty(keylist)==true
            keylist = cat(1,keylist,decrypt_key);
            occurencelist = cat(1,occurencelist,[1]);
        else
            [tf,index] = ismember(decrypt_key,keylist,'rows');
            if tf==true
               occurencelist(index) = occurencelist(index) + 1; 
            else
                keylist = cat(1,keylist,decrypt_key);
                occurencelist = cat(1,occurencelist,[1]);
            end
    ...
    [m,ind] = max(occurencelist);
    decrypt_key = keylist(ind,:);
    decrypted_txt = double2char(decrypt_key(char2double(encrypted_txt)));
    keylist
    occurencelist
